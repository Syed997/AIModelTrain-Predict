...
# ==================== 4. LOAD DATA & RUN ====================
data_norm, true_labels, anomaly_types = load_data("data/kafka_load.csv")
...
# print(f"Using FIXED production threshold: {threshold:.8f}")
mad = np.load("models/train_mad.npy")
print(f"Normal error level (from training): {mad:.8f}")

# Compute how many times worse than normal
relative_score = errors / mad
threshold = 10.0   # alert if 10× worse than normal

anomalies = relative_score > threshold
print(f"Detected anomalies: {anomalies.sum()} (only real ones!)")
